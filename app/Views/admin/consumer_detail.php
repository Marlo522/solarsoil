...
<!-- Back Button -->
<a href="<?= base_url('admin/consumers') ?>" class="inline-flex items-center gap-2 text-gray-500 hover:text-primary-700 mb-6 transition">
...
<div class="bg-white rounded-xl border border-gray-200 p-8 max-w-3xl">
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">#CMR<?= esc($consumer['user_id']) ?></h1>
            <h2 class="text-lg font-semibold text-gray-700 mt-2">Consumer Information</h2>
            <p class="text-gray-600 mt-1">Farmer Name: <?= esc($consumer['first_name'] . ' ' . ($consumer['middle_name'] ? $consumer['middle_name'] . ' ' : '') . $consumer['last_name'] . ($consumer['suffix'] ? ' ' . $consumer['suffix'] : '')) ?></p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
        <div>
            <p class="text-sm text-gray-900">
                ...
                <?= date('M j, Y', strtotime($consumer['date_joined'] ?? $consumer['created_at'] ?? 'now')) ?>
            </p>
            ...
            <p class="text-sm text-gray-900 mt-3">
                ...
                <?= esc($consumer['contact_number'] ?? 'N/A') ?>
            </p>
        </div>
        <div>
            <p class="text-sm text-gray-900">
