                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-1">
                                <a href="<?= base_url('admin/farmers/' . $farmer['user_id']) ?>" class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition" title="View">
                                ...
                                <?php if ($farmer['isActive']): ?>
                                <button @click="openToggle(<?= $farmer['user_id'] ?>, '<?= esc($farmer['first_name'] . ' ' . $farmer['last_name'], 'js') ?>', true)" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Deactivate">
                                ...
                                <?php else: ?>
                                <button @click="openToggle(<?= $farmer['user_id'] ?>, '<?= esc($farmer['first_name'] . ' ' . $farmer['last_name'], 'js') ?>', false)" class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition" title="Activate">
...
<div x-show="showToggleModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" @click="showToggleModal = false">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6" @click.stop>
        <div class="text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-2" x-text="toggleUser.isActive ? 'Deactivate Farmer' : 'Activate Farmer'"></h3>
        ...
        <div class="flex gap-3">
            ...
            <template x-if="toggleUser.isActive">
                <a :href="`<?= base_url('admin/farmers/deactivate/') ?>` + toggleUser.id" class="flex-1 px-4 py-2 text-sm font-medium text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition text-center">Deactivate</a>
            </template>
            <template x-if="!toggleUser.isActive">
                <a :href="`<?= base_url('admin/farmers/activate/') ?>` + toggleUser.id" class="flex-1 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition text-center">Activate</a>
            </template>
        </div>
    </div>
</div>
